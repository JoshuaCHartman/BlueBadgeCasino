...
Included in this application is full **CRUD** functionality, however most **PUT**s and **DELETE**s are fairly limited.  A player only has the ability to edit basic info like address and phone number.  When a player deletes their account, it is set to "inactive.".  In the unlikely event of an error, Admin can delete a bet, but not players or games.  When the bet is deleted, the accounts of the house and player are automatically adjusted accordingly.   
